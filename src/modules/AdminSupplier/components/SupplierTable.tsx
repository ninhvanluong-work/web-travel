...
export function SupplierTable({ suppliers, isLoading, isFetching, onDelete }: SupplierTableProps) {
    ...
    >
      <Table>
        ...
        <TableBody className="[&_td]:px-5 [&_td]:py-4">
          ...
          {!isLoading &&
            suppliers.length > 0 &&
            suppliers.map((supplier) => <SupplierTableRow key={supplier.id} supplier={supplier} onDelete={onDelete} />)}
        </TableBody>
      </Table>
    </div>
  );
