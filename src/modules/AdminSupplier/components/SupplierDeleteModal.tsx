...
interface SupplierDeleteModalProps {
  target: ISupplier | null;
  isDeleting: boolean;
  onConfirm: () => void;
  onCancel: () => void;
}

export function SupplierDeleteModal({ target, isDeleting, onConfirm, onCancel }: SupplierDeleteModalProps) {
      ...
      <AlertDialogContent className="rounded-2xl border-slate-100 shadow-2xl max-w-md dark:border-gray-800/80">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-slate-500 dark:text-gray-400">
            {t('confirmDeleteSupplierMessage', { name: target?.name })}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="flex flex-row justify-end gap-3 sm:space-x-0 mt-2">
          ...
          >
            ...
            {isDeleting ? t('deleting') : t('deleteSupplier')}
          </Button>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
